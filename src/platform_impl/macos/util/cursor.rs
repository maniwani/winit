...
use icrate::AppKit::{NSCursor, NSImage};
...
unsafe fn load_webkit_cursor(name: &NSString) -> Id<NSCursor> {
    ...
    let image = unsafe { NSImage::initByReferencingFile(NSImage::alloc(), &pdf_path) }.unwrap();
    ...
    unsafe { NSCursor::initWithImage_hotSpot(NSCursor::alloc(), &image, hotspot) }
}
...
pub(crate) fn invisible_cursor() -> Id<NSCursor> {
    ...
    static CURSOR: Lazy<Wrapper> = Lazy::new(|| {
        ...
        Wrapper(unsafe { NSCursor::initWithImage_hotSpot(NSCursor::alloc(), &image, hotspot) })
    });
